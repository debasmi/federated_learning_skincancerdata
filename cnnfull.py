...
def train_model(name, model, loaders, class_weights, device, epochs, lr):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[{name}] epoch {epoch:02d}/{epochs} | "
              ...
              f"val acc {val_m['accuracy']:.4f} | val AUC {val_m['auc_macro_ovr']:.4f}")
        # `>=` so a NaN AUC can't leave best_state as None
        auc = val_m["auc_macro_ovr"]
        if best_state is None or (not np.isnan(auc) and auc > best_auc):
            best_auc = auc if not np.isnan(auc) else best_auc
...
def main():
    ...
    all_results["UNet"] = train_model(
        ...
        args.epochs, args.lr_unet)
    all_results["ResNet18_Transfer"] = train_model(
        "ResNet18_Transfer", TransferResnet(), loaders, class_weights, device,
        args.epochs, args.lr_transfer)

    print("\n" + "=" * 78)
    print(f"{'Model':<20}{'Acc':>8}{'F1(macro)':>11}{'F1(wtd)':>9}{'AUC':>8}{'Time(s)':>10}{'Params(M)':>11}")
    print("-" * 78)
    for name, r in all_results.items():
        print(f"{name:<20}{r['accuracy']:>8.4f}{r['f1_macro']:>11.4f}{r['f1_weighted']:>9.4f}"
              f"{r['auc_macro_ovr']:>8.4f}{r['training_time_sec']:>10.1f}{r['params_millions']:>11.2f}")
    print("=" * 78)
